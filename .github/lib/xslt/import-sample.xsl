...
<xsl:stylesheet version="3.0"
    ...
    <xsl:template match="tei:table">
        <table class="{string-join(('table', @rend), ' ')}">
            <xsl:apply-templates mode="#current"/>
        </table>
    </xsl:template>
    
    <xsl:template match="tei:row">
        <tr class="{string-join(('tr', @rend), ' ')}">
            <xsl:apply-templates mode="#current"/>
        </tr>
    </xsl:template>
    
    <xsl:template match="tei:cell">
        <td class="{string-join(('td', @rend), ' ')}">
            <xsl:if test="@cols">
    ...
    <xsl:template match="tei:hi[not(@rend) or not(normalize-space(@rend))]">
        <xsl:apply-templates mode="#current"/>
    </xsl:template>
    
    <xsl:template match="tei:hi[normalize-space(@rend)]">
        <xsl:choose>
            ...
            <xsl:when test="@rend = ('italic', 'print')">
                <i>
                    <xsl:apply-templates mode="#current"/>
                </i>
            </xsl:when>
    ...
    <xsl:template name="teiEndHook">
        ...
        <script src="https://epsilon.ac.uk/script/stretchies.js"/>
        <script src="https://unpkg.com/prismjs@1.30.0/prism.js"/>
        <script type="text/x-mathjax-config">
            ...
            <xsl:text>MathJax.Hub.Register.StartupHook("End",dcp.renderStretchies());</xsl:text>
        </script>
        <script src="https://cdn.jsdelivr.net/npm/mathjax@2/MathJax.js" async="async"/>
    </xsl:template>
